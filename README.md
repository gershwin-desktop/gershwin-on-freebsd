...
Build system for generating Gershwin Desktop live media based on FreeBSD.
...
- A FreeBSD or FreeBSD host system.
...
The live system uses a memory-efficient `uzip` and `nullfs` layering architecture. The default user is `admin` with no password required.
